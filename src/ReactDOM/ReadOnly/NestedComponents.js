...
describe('Modal component', () => {
  ...
  it('should trigger toggle when clicked', () => {
    ...
    expect(toggle.mock.calls.length).toHaveLength(1);
    expect(toggle.mock.calls[0][0]).toBeFalsy();
  });

  xit('should mount modal on the div with id modal-root', () => {
  ...
  xit('should clear the div with id modal-root on unmount', () => {
    const modalRoot = global.document.querySelector('#modal-root');
  ...
  xit('should set overflow hidden on the boddy element', () => {
    const body = global.document.querySelector('body');
